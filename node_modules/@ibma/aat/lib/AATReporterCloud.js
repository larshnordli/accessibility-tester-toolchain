...
var Config = require("./AATConfigLoader");
var request = require("request");

// Variable Declearation
var log;

// Global AAT Summary Holder
var scanSummary = {};

/**
 * This function is responsible for constructing the AAT Reporter which will be used to, report
 ...
 * @memberOf this
 */
var AATReporter = function () {
    Config.DEBUG && console.log("START AATReporter Constructor");
    // Override adapters
    this.adapters = [];
    ...
    this.addScreenshot = function (screenshotResult) {
        // https://aat.w3ibm.mybluemix.net/token/a7b86fd6-9979-428a-9bed-910035015b19/047764ba-b253-4e08-ac27-6fc1fcc6c0ce/dap/js/latest/
        var rulePack = Config.rulePack;
        var m = rulePack.match(/^(https:\/\/[^/]*)(.*\/)dap\/js\/latest\/$/);
        var url = m[1] + "/api" + m[2] + "screenshot";
        request({
            url: url,
            method: "POST",
     ...
     * saved.
     *
     * @param {Object} config - Karma config object, used to extrat the outputFolder from the AATConfig.
    ...
    var saveSummary = function (summary, done) {
        if (Config.outputFormat.indexOf("cloud") === -1) {
            done && done();
            return;
        }
        ...
        // Extract the outputFolder from the AATConfig (this is the user config that they provid)
        ...
        var month = datePadding(formattedData.getUTCMonth());
        ...
        var m = rulePack.match(/^(https:\/\/[^/]*)(.*\/)dap\/js\/latest\/$/);
        if (m && m[1] && m[2]) {
            var url = m[1]+"/api"+m[2]+"scanSummary";
            Config.DEBUG && console.log("SEND 'saveSummary'");
            request({
                url: url,
                method: "POST",
                json: summary
            }, function(err, res, body) {
                Config.DEBUG && console.log("SEND RESULT 'saveSummary'", err, res);
                done && done();
            });
        } else {
            done && done();
    ...
    var addToSummaryCount = function (pageCount) {
        ...
        // Variable Decleration
        var AATScanSummary = scanSummary.counts || {};
        ...
        scanSummary.counts = AATScanSummary;
    }
    ...
    Config.DEBUG && console.log("END AATReporter Constructor");
};

// Export this function, which will be called when Karma loads the reporter
module.exports = AATReporter;
